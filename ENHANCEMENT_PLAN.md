...
Before building algorithmic trading features (Phase A), the SDK's existing resilience and production-readiness infrastructure must be fully wired and verified. Items in this phase ensure that rate limiting, retry, WebSocket reconnect, and other features are not just present in the codebase but actually functional in the request path.
