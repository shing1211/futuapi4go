...
| `GetHistoryKLQuota(ctx, c)` | API 配额使用情况 |
...
| 函数 | 描述 |
|---|---|
| `Subscribe(ctx, c, market, code, []constant.SubType)` | 订阅推送类型 |
| `Unsubscribe(ctx, c, market, code, []constant.SubType)` | 取消订阅 |
